...
class MatchGame extends Component {
  ...
  timerFunc = () => {
    ...
    if (elapsedSeconds > 0) {
      ...
      clearInterval(this.timerId)
      this.setState({isGameInProgress: false, elapsedSeconds: 0})
    }
  }

  onClickTabItem = tabId => {
    const currentTab = tabsList.find(eachTab => eachTab.tabId === tabId)

    this.setState({activeTabId: currentTab.tabId})
  }
  ...
  render() {
    ...
    return (
      <div className="match-game-container">
        <Navbar currentScore={currentScore} elapsedSeconds={elapsedSeconds} />
